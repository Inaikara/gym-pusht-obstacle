...
def main():
    ...
    # 创建HDF5文件用于保存所有演示数据
    h5_filename = "pusht_demonstrations.hdf5"
    ...
    try:
        while running:
            ...
            if teleop or distance < 30:
                ...
                # 保存当前状态
                observations['pixels'].append(observation.get('pixels', np.zeros((84, 84, 3), dtype=np.uint8)))
                observations['agent_pos'].append(info.get('pos_agent', np.zeros(2)))
